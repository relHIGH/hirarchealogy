...
**任何涉及布局/样式/CSS的调整，必须遵循以下流程：**

1. **修改前**：截图留存当前状态作为对比基准
2. **修改后**：生成截图验证视觉效果
3. **对比确认**：确保修改符合预期，无意外变化
4. **用户确认**：截图核验无误后再提交给用户

**截图工具**：使用 Chrome Headless 或 Playwright 生成页面截图
**截图保存位置**：`visual_check/` 目录
