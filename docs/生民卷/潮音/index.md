---
layout: default
title: 潮音
---

<div class="story-page">
  <div class="story-header">
    <div class="char-avatar" style="background-position: 0% 100%;"></div>
    <div class="char-title">
      <h1>潮音</h1>
      <p>五代十国 · 吴越运河船家女</p>
    </div>
  </div>
  
  <div class="story-timeline">
    <div class="story-chapter">
      <h3>第一章：运河人家</h3>
      <p>潮音生于[[杭州]]城外的[[运河]]畔，祖祖辈辈以[[漕运]]为生。 [[江南运河]]贯通南北，船只往来如织。</p>
      <p>她自幼在[[货船]]上长大，熟悉每一条[[水闸]]、每一处[[码头]]。</p>
    </div>
    
    <div class="story-chapter">
      <h3>第二章：吴越水乡</h3>
      <p>吴越国偏安江南，[[钱镠]]王修筑海塘，开辟[[航道]]。潮音的[[漕船]]运送着[[稻米]]、[[丝绸]]、[[茶叶]]，连接着散落在水网中的城镇。</p>
      <p>她听过[[越剧]]的悠扬，见过[[龙井]]的翠绿。</p>
    </div>
    
    <div class="story-chapter">
      <h3>第三章：涛声依旧</h3>
      <p>宋统一江南后，运河依旧繁忙。潮音摇着[[橹]]，在[[钱塘江]]的潮声中，继续着她水上的生涯。</p>
    </div>
  </div>
  
  <div class="story-collection" id="collection-section">
    <h2>探索收藏</h2>
    <div class="collection-grid">
      <div class="collection-group">
        <h4>主题节点</h4>
        <div class="collected-nodes" id="nodes-lit"></div>
      </div>
      <div class="collection-group">
        <h4>历史文物</h4>
        <div class="collected-relics" id="relics-lit"></div>
      </div>
    </div>
    
    <div class="export-actions">
      <button onclick="Shengmin.exportImage()">生成探索证书</button>
      <button onclick="Shengmin.exportLink()">复制分享链接</button>
    </div>
  </div>
</div>

<script>
Shengmin.storyData = {
  character: '潮音',
  nodes: ['杭州', '运河', '漕运', '江南运河', '货船', '水闸', '码头', '钱镠', '航道', '漕船', '稻米', '丝绸', '茶叶', '越剧', '龙井', '橹', '钱塘江'],
  relics: ['丝绸', '茶叶']
};
</script>
